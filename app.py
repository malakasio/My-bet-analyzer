...
_DEFAULTS: dict = {
    ...
    "fetch_summary": [],        # human-readable list of what was updated
...
def _af(endpoint: str, params: dict, api_key: str) -> dict:
    """
    Cached API-Football GET. TTL = 30 min for successful responses.
    Error responses are NOT cached (st.cache_data skips caching on exception),
    so a bad key or a typo in team name will not persist in cache.
    """
    headers = {
    ...
    try:
        ...
        if r.status_code == 401:
            # Raise so st.cache_data does NOT cache this result
            raise ValueError("Invalid API key (HTTP 401). Check your RapidAPI key.")
        if r.status_code == 429:
            raise ValueError("Daily API limit reached (100 req/day on free tier).")
        r.raise_for_status()
        data = r.json()
        # API-Football returns errors in the 'errors' dict even with HTTP 200
        if data.get("errors"):
            err_msg = str(list(data["errors"].values())[0])
            raise ValueError(f"API-Football error: {err_msg}")
        return data
    except requests.exceptions.Timeout:
        raise ValueError("Request timed out after 12 s — check your internet connection.")
...
def af_search_team(name: str, api_key: str) -> dict | None:
    ...
    Returns {"error": msg, "team": None} if the API itself threw an error.
    """
    try:
...
def do_auto_fetch(football_key: str, odds_key: str) -> None:
    ...
    No UI elements created here — all results stored in session_state
    so they persist after st.rerun().
    """
    ...
    log.append("🔍 Searching for teams…")
    ta_data = af_search_team(ta, football_key)
    tb_data = af_search_team(tb, football_key)

    # Check for API key / network errors (sentinel dict returned by af_search_team)
    ...
    if not errors:
        if ta_data is None:
            errors.append(
                f"❌ Team not found: '{ta}'\n"
                "   → Use the exact English name as on API-Football "
                "(e.g. 'Juventus', 'Manchester City', 'Atletico Madrid').\n"
                "   → Try the team page on api-football.com to confirm the spelling."
            )
        else:
    ...
    if fix_a:
        ...
        errors.append(f"❌ No recent finished fixtures found for {ta}. "
                      "Season may not have started or API tier does not cover this league.")
    ...
    if odds_key.strip():
        ...
        if isinstance(games, list) and games and "error" in games[0]:
            log.append(f"⚠️  Odds API error: {games[0]['error']}")
            ...
            if match_game:
                ...
                log.append(
                    f"⚠️  Match not in Odds API for sport key '{sport_key}'. "
                    "Game may not be listed yet (usually appears 2–3 days before kickoff)."
                )
    else:
        log.append("ℹ️  No Odds API key — skipping odds fetch.")
