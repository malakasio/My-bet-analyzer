...
_DEFAULTS: dict = {
    ...
    # Names that were active when IDs were last confirmed — used to detect staleness
    "team_a_id_name": "", "team_b_id_name": "",
    # Checklist
...
def af_search_all(name: str, api_key: str) -> list:
    ...
    Raises ValueError on API / network errors so st.cache_data does NOT cache
    failures (an empty list would be indistinguishable from "no results found"
    and would block retries for up to 5 minutes).
    """
...
def af_last_fixtures(team_id: int, n: int, api_key: str) -> list:
    ...
    Rate-limit and auth errors are re-raised so callers can surface them
    to the user instead of silently returning an empty list.
    """
    ...
    def _is_fatal(exc: ValueError) -> bool:
        """
        True for errors that must surface immediately (rate limit, auth failure).
        These cannot be recovered by retrying with a different parameter set.

        Intentionally narrow — avoid substring 'key' or 'invalid' alone because
        API error messages like 'Parameter key is invalid' would wrongly classify
        a per-endpoint error as a global auth failure, preventing fallback attempts.
        """
        msg = str(exc).lower()
        return (
            "429" in msg
    ...
    try:
        ...
        if _is_fatal(exc):
            raise  # Surface rate-limit / auth errors immediately
...
def af_parse_fixtures(fixtures: list, team_id: int) -> dict:
    ...
    Defensive against missing or None 'goals'/'teams' keys — malformed rows
    are skipped rather than crashing the entire fetch.
    """
    ...
    for f in fixtures:
        try:
            ...
            continue  # skip any other malformed structure silently

    return {
        "form": form,
        "clean_sheets": clean_sheets,
        "avg_scored":   round(scored_total   / used, 2) if used else 0.0,
        "avg_conceded": round(conceded_total / used, 2) if used else 0.0,
    }
...
def do_auto_fetch(football_key: str, odds_key: str) -> None:
    ...
    pre_a_name = st.session_state.get("team_a_id_name", "")
    pre_b_name = st.session_state.get("team_b_id_name", "")

    # Invalidate picker IDs if the team names have changed since they were confirmed.
    # Without this check, changing "Napoli" to "Juventus" would still fetch Napoli's data.
    if pre_a and pre_a_name != ta:
        log.append(
            f"⚠️ Team A name changed ('{pre_a_name}' → '{ta}') since last picker selection "
            f"— stale ID {pre_a} discarded."
        )
        pre_a = None
    if pre_b and pre_b_name != tb:
        log.append(
            f"⚠️ Team B name changed ('{pre_b_name}' → '{tb}') since last picker selection "
            f"— stale ID {pre_b} discarded."
        )
        pre_b = None
    ...
    try:
        ...
        errors.append(
            f"API error fetching fixtures for {ta}: {exc}  "
            "If this is a rate-limit error, your 100 free requests/day are used up. "
            "Click 'Clear API Cache' in the sidebar and try again tomorrow, "
            "or fill the Statistics and Forms tabs manually."
        )
        _finish()
        return
    try:
        ...
        errors.append(
            f"API error fetching fixtures for {tb}: {exc}  "
            "If this is a rate-limit error, your 100 free requests/day are used up."
        )
        _finish()
        return
    ...
    if not fix_a and not fix_b:
        errors.append(
            "No finished fixtures found for either team in the last 120 days. "
            "Possible causes: (1) daily API limit reached (100 req/day) — click 'Clear API Cache' "
            "in the sidebar, wait until midnight UTC, then retry; "
            "(2) this league's fixture data requires a paid API-Football plan; "
            "(3) the season just started and no matches have been played yet. "
            "You can fill the Statistics and Forms tabs manually in the meantime."
        )
        _finish()
        return
...
def _ta() -> str:
    """Return Team A name, HTML-escaped for safe embedding in unsafe_allow_html blocks."""
    return _html.escape(st.session_state.team_a or "Team A")

def _tb() -> str:
    """Return Team B name, HTML-escaped for safe embedding in unsafe_allow_html blocks."""
    return _html.escape(st.session_state.team_b or "Team B")
...
def run_analysis() -> dict:
    ...
    # ── Verdict ─────────────────────────────────────────────
    #
    # Five possible verdicts, all grounded in the bet file:
    #
    # UNDER 1.5 Goals (~90%)
    #   Bet file Section 4.2 — "almost 90% success ONLY IF" (3 conditions)
    #
    # OVER 2.5 Goals
    #   Bet file Section 6 Motivation Matrix:
    #     "One team in top 3 or bottom 4? → HIGH Over risk →
    #      Relegation/title pressure = chaos = more goals"
    #   Bet file Section 2 (high-variance leagues):
    #     "emotional, chaotic play" → more goals
    #   Also fires when offensive formation confirmed (attacking overload).
    #   Requires must_abort=True AND at least one of those signals.
    #
    # ABORT ALL UNDER BETS (no Over signal either)
    #   Under must be aborted, but the only reason is defensive integrity.
    #   No signal pointing to more goals → skip the match.
    #
    # UNDER 2.5 Goals
    #   Bet file Section 5 example — Fiorentina vs Napoli, tip = UNDER 2.5
    #
    # NO CLEAR SIGNAL
    #   Medium-variance or inconclusive data — skip the match.

    over_signal = is_high_var or high_stakes or offensive_formation
    ...
    if transfer_rule and not must_abort:
        ...
        rationale = ("New striker starting + both teams ≥ 2 clean sheets + low-variance league. "
                     "3-condition rule — ~90% historical accuracy per the methodology.")

    elif must_abort and over_signal:
        verdict, css, base_confidence = "OVER 2.5 Goals", "verdict-over", 60
        over_reasons = []
        if is_high_var:
            over_reasons.append("high-variance league (≥2.6 g/m) — emotional, chaotic play")
        if high_stakes:
            over_reasons.append("relegation/title pressure — chaos = more goals")
        if offensive_formation:
            over_reasons.append("offensive formation confirmed (2+ strikers) — attacking overload")
        rationale = "Over signals: " + "; ".join(over_reasons) + ". Under bets aborted."

    elif must_abort:
        verdict, css, base_confidence = "ABORT ALL UNDER BETS", "verdict-abort", 0
        rationale = ("Defensive integrity failure — no clear Over signal. "
                     "Under is aborted but no positive signal for Over either. Skip this match.")
        ...
        rationale = ("Low-variance league, both teams defensively solid, "
                     "no high-stakes table pressure, no offensive formation overload.")

    else:
        verdict, css, base_confidence = "NO CLEAR SIGNAL", "verdict-no-signal", 35
        rationale = (
            "Conditions not clearly aligned for Under or Over. "
            "Medium-variance league or insufficient data — skip this match."
        )
    ...
    analyst_reports = [
        ...
        {"analyst": "📈 Forms & Motivation", "lines": [
            ...
            f"High-stakes pressure: {'🔴 Yes — OVER signal (relegation/title pressure)' if high_stakes else '🟢 None'}",
        ]},
...
with st.sidebar:
    ...
    if not picker_key:
        ...
        if st.button("🔍 Search Both Teams", use_container_width=True):
            ...
            with st.spinner("Searching…"):
                ...
                try:
                    ...
                    st.error(f"Search failed for '{ta_q}': {exc}")
                try:
                    rb = af_search_all(tb_q, picker_key) if tb_q else []
                except ValueError as exc:
                    st.error(f"Search failed for '{tb_q}': {exc}")
        ...
        if st.session_state.team_a_search_results:
            ...
            # Snapshot the current team name so staleness can be detected later
            st.session_state.team_a_id_name = st.session_state.team_a.strip()
            st.success(
                f"✅ {chosen_a['team']['name']} — {chosen_a['team']['country']} "
                f"(ID {chosen_a['team']['id']})"
            )
        elif st.session_state.team_a_id:
            if st.session_state.team_a_id_name != st.session_state.team_a.strip():
                st.warning(
                    f"⚠️ Team A name changed since last pick — ID may be stale. "
        ...
        if st.session_state.team_b_search_results:
            ...
            st.session_state.team_b_id_name = st.session_state.team_b.strip()
            st.success(
                f"✅ {chosen_b['team']['name']} — {chosen_b['team']['country']} "
                f"(ID {chosen_b['team']['id']})"
            )
        elif st.session_state.team_b_id:
            if st.session_state.team_b_id_name != st.session_state.team_b.strip():
                st.warning(
                    f"⚠️ Team B name changed since last pick — ID may be stale. "
                    "Search again to confirm."
                )
            else:
    ...
    # ── Clear cache button ─────────────────────────────────
    if st.button("🗑️ Clear API Cache", use_container_width=True,
                 help="Forces fresh API calls on next fetch by clearing cached results. "
                      "Use this if the fetch returned empty or wrong data."):
        st.cache_data.clear()
        st.success("✅ Cache cleared — next fetch makes fresh API calls.")
...
with t_verdict:
    ...
    if st.button("⚙️  Run Full Analysis", type="primary", use_container_width=True):
        ...
        st.markdown(
            _alert("info", "Verdict — based on the bet file methodology",
                   "<strong>UNDER 1.5 Goals</strong> (~90%) — transfer rule, 3 conditions met (Section 4.2)  ·  "
                   "<strong>OVER 2.5 Goals</strong> — abort conditions active + high-variance / high-stakes / offensive formation (Section 6 Motivation Matrix &amp; Section 2)  ·  "
                   "<strong>ABORT ALL UNDER BETS</strong> — defensive integrity failure only, no Over signal  ·  "
                   "<strong>UNDER 2.5 Goals</strong> — all conditions clear (Section 5 Fiorentina–Napoli example)  ·  "
                   "<strong>NO CLEAR SIGNAL</strong> — medium variance or insufficient data — skip the match"),
            unsafe_allow_html=True,
        )
        ...
        with st.expander("View complete rule evaluation"):
            ...
            abort_all = not bc or hv or hs_a or hs_b or st.session_state.tac_a_offensive or st.session_state.tac_b_offensive
            st.markdown(f"""
**Abort conditions** (any ❌ here → ABORT ALL UNDER BETS):

| Bet-file rule | Condition required | Status |
|---------------|--------------------|--------|
| Defensive Integrity | Both teams ≥ 2/5 clean sheets | {t(bc)} |
| League Volatility | Low-variance league (< 2.4 g/m) | {t(not hv)} |
| Motivation — {ta} | Not top-3 or bottom-4 | {t(not hs_a)} |
| Motivation — {tb} | Not top-3 or bottom-4 | {t(not hs_b)} |
| Confirmed Lineup — {ta} | No offensive formation (2+ strikers) | {t(not st.session_state.tac_a_offensive)} |
| Confirmed Lineup — {tb} | No offensive formation (2+ strikers) | {t(not st.session_state.tac_b_offensive)} |

**Positive tip conditions**:

| Bet-file rule | Condition | Status |
|---------------|-----------|--------|
| Under 1.5 rule | New striker starts + both CS + low-var | {t(result['transfer_rule'])} |
| Under 2.5 conditions | Low-var + solid defence + mid-table + no offensive formation | {t(lv and bc and not hs_a and not hs_b and not st.session_state.tac_a_offensive and not st.session_state.tac_b_offensive)} |

**Warning signals** (affect reasoning but not verdict):

| Signal | Triggered |
|--------|-----------|
| Bookmaker odds trap (80% inverse rule) | {t(st.session_state.bm_odds_dropped)} |
| Manager friendship / relaxed game | {t(st.session_state.mgr_friends and (st.session_state.mgr_a_needs_points != st.session_state.mgr_b_needs_points))} |
| Social life disruption | {t(any([st.session_state.soc_a_nightclub, st.session_state.soc_a_training, st.session_state.soc_a_feud, st.session_state.soc_b_nightclub, st.session_state.soc_b_training, st.session_state.soc_b_feud]))} |

**Research coverage**: {result['checklist_verified']}/5 areas verified  |  Live API data: {t(fetched)}
            """)
...
**5 possible verdicts:**
- 🟢 **UNDER 1.5 Goals** (~90%) — new striker + both CS + low-variance
- 🔴 **OVER 2.5 Goals** — high-variance / high-stakes / offensive formation
- 🟠 **ABORT ALL UNDER BETS** — defensive failure only, no Over signal → skip
- 🟢 **UNDER 2.5 Goals** — all conditions clear, low-variance, solid defence
- ⚪ **NO CLEAR SIGNAL** — medium variance / inconclusive → skip
        """)
