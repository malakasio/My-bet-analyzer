...
def af_last_fixtures(team_id: int, n: int, api_key: str) -> list:
    ...
    Tries status=FT first; falls back to all statuses filtered client-side
    so leagues that use different status codes (AET, PEN) are also captured.
    """
    try:
        # Primary: strict FT filter
        d = _af("fixtures", {"team": team_id, "last": n, "status": "FT"}, api_key)
        fixtures = d.get("response", [])
        if fixtures:
            return fixtures
        # Fallback: no status filter, collect more and filter client-side
        d = _af("fixtures", {"team": team_id, "last": n * 3}, api_key)
        all_fix = d.get("response", [])
        finished = [
            f for f in all_fix
